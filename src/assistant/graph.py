...
def send_to_teams(state: SummaryState, config: RunnableConfig):
    cfg = Configuration.from_runnable_config(config)
    if not cfg.teams_webhook_url:
        raise ValueError("Teams webhook not configured.")
    import requests
    payload = {"text": state.running_summary}
    r = requests.post(cfg.teams_webhook_url, json=payload)
    r.raise_for_status()
    return {"teams_posted": True}
...
builder.add_node("reddit_research", reddit_search)
builder.add_node("init_datastores", init_datastores)
builder.add_node("store_mongo", store_in_mongo)
builder.add_node("upsert_pinecone", upsert_to_pinecone)
builder.add_node("summarize_sources", summarize_sources)
builder.add_node("reflect_on_summary", reflect_on_summary)
builder.add_node("finalize_summary", finalize_summary)
builder.add_node("send_to_teams", send_to_teams)  # new node for emailing the summary

# Add edges
builder.add_edge(START, "generate_query")
builder.add_edge("generate_query", "web_research")
builder.add_edge("web_research", "youtube_research")  # route to YouTube research after web research
builder.add_edge("youtube_research", "reddit_research")
builder.add_edge("reddit_research", "init_datastores")
builder.add_edge("init_datastores", "store_mongo")
builder.add_edge("store_mongo", "upsert_pinecone")
builder.add_edge("upsert_pinecone", "summarize_sources")
builder.add_edge("youtube_research", "summarize_sources")
builder.add_edge("summarize_sources", "reflect_on_summary")
builder.add_conditional_edges("reflect_on_summary", route_research)
builder.add_edge("finalize_summary", "send_to_teams")
